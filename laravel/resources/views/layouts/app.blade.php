...
<head>
    ...
    <title>@yield('title', 'Accentra')</title>
    <style>
...
<body>
    <nav class="nav">
        <a class="brand" href="{{ route('home') }}"><span class="mark">A</span> Accentra</a>
        <div class="links">
            <a href="{{ route('courses.index') }}">Courses</a>
            @auth
                <a href="{{ route('dashboard') }}">Dashboard</a>
                <a href="{{ route('speaking.practice') }}">Speaking</a>
                <a href="{{ route('tutor.index') }}">Tutor</a>
    ...
    <main>
        @if(session('status'))
            <div class="container" style="padding-top:18px"><div class="alert">{{ session('status') }}</div></div>
        @endif
        @yield('content')
    </main>
